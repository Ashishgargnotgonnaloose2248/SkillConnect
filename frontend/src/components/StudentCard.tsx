...
export default function StudentCard({ student }: { student: Student }) {
  ...
  const [template, setTemplate] = useState<string>(
    "Hi! I'd like to connect for a quick session.",
  );
    ...
    <Card className="group rounded-2xl border bg-card p-5 shadow-sm transition hover:shadow-md">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex h-2.5 w-2.5 rounded-full ${student.available ? "bg-brand-green" : "bg-brand-red"}`}
          />
          <span className="text-xs text-muted-foreground">
            {student.available ? "Available" : "Offline"}
          </span>
        </div>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        {student.skills.map((s) => (
          <span
            key={s}
            className="inline-flex items-center rounded-full bg-brand-sky/10 text-brand-blue px-2.5 py-1 text-xs ring-1 ring-brand-blue/20"
          >
            {s}
          </span>
        ))}
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
          {student.slots.length === 0 ? (
            ...
            student.slots.map((t) => (
              <span key={t} className="rounded-lg border px-2 py-1">
                {t}
              </span>
            ))
          )}
            ...
            <div className="space-y-3">
              <div>
                ...
                >
                  ...
                  <option>
                    I need help with {student.skills[0] || "your skill"}. Are
                    you available?
                  </option>
                </select>
              </div>
